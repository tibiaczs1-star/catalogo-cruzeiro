...
- `script.js` separou as fases: espera estrutura/carregamento real, inicia a abertura cinematografica e conta 4,5s a partir da animacao. Quando a abertura começa, a home ja fica pronta por baixo e o CSS tem saida automatica da overlay para nao depender de JS atrasado no Render. A intro continua sendo por sessao do navegador e a navegacao interna continua com barra curta, nao full-screen.
...
- `script.js`: a home editorial no desktop agora pula o splash inicial; mobile continua preservado.
- `startup-experience.js`: a home editorial no desktop pula o prelude fundador e nao exibe o loader "Abertura do portal"; depois do aceite de cookies tambem nao entra loader intermediario.
- `premium-clarity.css`: chips do Radar e etiquetas/plaquinhas de categoria ganharam `nowrap`, largura real e sem ellipsis; a faixa `Mesa ao vivo` no desktop deixou de ser marquee com recortes laterais e passou a mostrar cards estaticos com categoria inteira.
- `index.html`: cache-bust de `premium-clarity.css`, `startup-experience.js` e `script.js` atualizado para a rodada desktop-open-chipfix.
- Validacoes: `node --check script.js`, `node --check startup-experience.js`, brace-balance `premium-clarity.css 332/332` e `styles.css 3446/3446`, Playwright desktop 1365x768 com `ok=true`, sem splash/prelude/loader, sem loader apos aceite e sem labels cortados; `npm run review:team` com `totalIssues=0`.
- Capturas: `output/playwright/desktop-open-chipfix-home-20260429.png` e `output/playwright/desktop-open-chipfix-radar-20260429.png`.

## Atualizacao rapida 2026-04-29 - PubPaid retomada local do transito pixel art

- Usuario reforcou a regra global: PubPaid 2.0 segue somente local/teste; nao fazer deploy, push ou commit sem ordem explicita.
- Retomada corrigida para o ponto real: triagem do transito pixel art da rua. O pacote WIP foi restaurado do checkpoint local `3f7110318e65b26d35bcbb728b5918bf022620cf`, trazendo `assets/pubpaid/traffic/`, runtime Phaser e scripts de pipeline.
- `StreetScene.js`: `TRAFFIC_ENABLED` foi ajustado para `true`, porque o trafego ja nascia no runtime por `nextSpawnAt = 0`; a mudanca remove a contradicao sem mudar o objetivo da triagem.
...
- A pedido do usuario, o pacote antigo deixou de ser a base da avaliacao: criado `scripts/build-pubpaid-traffic-redraw-left-3f-v1.py`, que desenha do zero uma folha `pubpaid-traffic-redraw-left-3f-v1.png` com 10 veiculos, direcao unica direita->esquerda, 3 frames por veiculo, rodas com centro fixo e contornos fortes. O runtime local passou a carregar esse sheet e `StreetScene.js` ficou com uma faixa so direita->esquerda. Validacoes: `semi=0`, `holes=0`, `node --check` ok e captura do jogo real em `output/web-game/pubpaid-traffic-check/06-redraw-left-vehicles-forced-scaled.png`.
...
- Loader real e atalhos uteis em 2026-05-13: por feedback do usuario, a home voltou a abrir com sequencia real de entrada: tela inicial limpa com logo, splash editorial com baloes de noticias/frases e interacao seguindo o mouse, depois loader que segura a home invisivel ate liberar `site-loaded`. Os cards de utilidade/checagem/comunidade foram reescritos como acoes reais: fontes usadas, pedir correcao, telefones urgentes, saude, chuva/rio, farmacia, WhatsApp para foto/video e sugestao de pauta. Validacoes: `new Function(script.js)`, contagem de chaves CSS ok, Playwright local confirmou `.page-shell` hidden/opacity 0 durante boot, splash visivel com 12 baloes, pointer ativo e final com splash `display:none`, home visivel/opacity 1; `node scripts/guard-pubpaid-no-canvas.js` OK; `node scripts/review-team-audit.js` totalIssues=0. Capturas: `output/playwright/home-real-loader-20260513.png` e `output/playwright/home-real-loader-final-20260513.png`.
- Carteira mobile do PubPaid 2.0 corrigida em 2026-05-16 para o teste real em dois celulares: `.ppg-dom-wallet` virou painel absoluto com rolagem touch propria, layout responsivo em coluna no breakpoint mobile e o QR Pix recem-gerado agora entra na area visivel via `scrollIntoView()`. O bug anterior era estrutural: sem `position`, o painel passava da altura util da tela e o deposito ficava inalcancavel no celular. Validacoes: `node --check pubpaid-phaser/ui/walletInterface.js`, `brace-balance=0` em `pubpaid-phaser.css`, `npm run guard:pubpaid`, Playwright portrait/landscape com scroll funcional e capturas `output/playwright/pubpaid-wallet-mobile-portrait-after-20260516.png`, `pubpaid-wallet-mobile-landscape-after-20260516.png`, `pubpaid-wallet-mobile-portrait-qr-20260516.png`, `pubpaid-wallet-mobile-landscape-qr-20260516.png`.
- Ajuste complementar da PubPaid 2.0 em 2026-05-16: o atalho de carteira saiu de `Esc` e passou para `Enter` para nao disputar com saida de fullscreen; o breakpoint responsivo da carteira foi ampliado ate 960px para cobrir celulares de viewport CSS larga; e Damas sem saldo real agora pede confirmacao antes de entrar em demo com a mensagem `Voce esta sem saldo real. Quer usar o saldo demo?`. Validacoes: `node --check` em `pubpaid-phaser/app.js`, `pubpaid-phaser/ui/domGameInterface.js` e `pubpaid-phaser/scenes/GameLobbyScene.js`, `npm run guard:pubpaid`, captura `output/playwright/pubpaid-wallet-839-enter-demo-confirm-20260516.png`.
- PubPaid 2.0 realfix em 2026-05-16 apos feedback do teste real: carros aprovados tiveram o `flipX` invertido para nao parecerem de re; `Enter` agora abre carteira em captura no `document` e a porta usa `Espaco`; abrir carteira seta `walletOpen` e força o protagonista para `idle-phone`, enquanto idle automatico so puxa celular depois de 5s e com frames mais lentos; Google Port tenta `refresh()` + `promptSignIn()` mesmo quando a tela estava marcada como teste local; Damas sem saldo real nao inicia mais `checkers-game-scene` e cai na confirmacao demo. Validacoes: `node --check` em `app.js`, `gameState.js`, `domGameInterface.js`, `walletInterface.js`, `StreetScene.js`, `InteriorScene.js`, `GameLobbyScene.js`; `npm run guard:pubpaid`; `brace-balance=0`; Playwright local `output/playwright/pubpaid-realfix-final-local-20260516.png`.
- PubPaid 2.0 hotfix `20260516-google-wallet-realflow2` em 2026-05-16: removido de vez o metodo Google de dentro do jogo/rua (`syncStreetGoogleGate` e chamadas), mantendo Google apenas como gate externo antes do jogo. `pubpaid-v2.html` e `BootScene.js` receberam novo cache-bust; `pubpaid-phaser.css` voltou a respeitar `hidden` no shell do jogo; o aviso mobile de orientacao nao cobre mais o gate Google inicial. Validacoes: `node --check` em `app.js` e `BootScene.js`, CSS `brace-balance=0`, `npm run guard:pubpaid`, Playwright desktop/mobile com capturas em `output/playwright/pubpaid-google-port-hotfix/flow/`, confirmando gate Google, carteira com nome/email/sub, deposito pendente, Damas bloqueada sem saldo real e rua sem Google Port.
- PubPaid 2.0 ajuste `20260516-google-wallet-invitecopy2` em 2026-05-16: carteira voltou a pedir `Nome no comprovante Pix` como dado de conferencia para o admin, sem virar fonte de identidade da carteira. Deposito so pode ser avisado depois de QR + nome no comprovante; backend exige `receiptName`; admin mostra nome Google, email, ID Google, valor Pix, status, referencia e nome no comprovante. Textos publicos removeram `Google primeiro`, `Aguardando Google`, `Google id` e linguagem tecnica da interface do jogador. Validacoes locais: `node --check`, `npm run guard:pubpaid`, CSS/admin brace balance e Playwright desktop/mobile com capturas em `.codex-temp/pubpaid-receiptname/`.

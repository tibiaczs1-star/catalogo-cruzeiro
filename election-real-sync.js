...
(function () {
  ...
  function resolveCandidatePhoto(candidate) {
    if (!candidate) return "";
    return (
      candidate.photoUrl ||
      candidate.imageUrl ||
      ...
      candidate.photo ||
      candidate.picture ||
      candidatePhotoFallbacks[candidate.id] ||
      ""
    );
  }
  ...
  async function loadScope(scope) {
    ...
    try {
      ...
      if (window.ELECTIONS_DATA) {
        const offices = Array.isArray(window.ELECTIONS_DATA.offices)
          ? window.ELECTIONS_DATA.offices.filter(
              (office) => String(office.scope || "").toLowerCase() === scope
            )
          : [];
        return {
          scope,
          updatedAt: window.ELECTIONS_DATA.updatedAt,
          summary: window.ELECTIONS_DATA.sourceNote,
          offices
        };
      }
      throw error;
  ...
  function patchLegacyElectionCards(candidates) {
    ...
    uniqueCards.forEach((card, index) => {
      ...
      const photoUrl = resolveCandidatePhoto(candidate);
      if (avatar && photoUrl) {
        avatar.classList.add("real-candidate-photo");
        avatar.innerHTML = `<img src="${photoUrl}" alt="Foto de ${candidate.name}" loading="lazy">`;
      }
  ...
  function renderCards(container, payload) {
    ...
    const pollingSource = safeText(payload?.polling?.source, 180);
    const pollingDate = safeText(payload?.polling?.date, 80);
    const pollingNote = pollingSource
      ? `Pesquisa de intenção de votos: ${pollingSource}${pollingDate ? ` • ${pollingDate}` : ""}.`
      : "Pesquisa de intenção de votos: aguardando instituto oficial publicar dados válidos.";

    summary.textContent =
      safeText(payload?.summary, 280) ||
      `Dados eleitorais públicos para facilitar comparação e voto consciente. ${pollingNote}`;
    ...
    grid.innerHTML = offices
      .map((office) => {
        ...
        const candidateHtml = candidates
          .map((candidate) => {
            const photoUrl = resolveCandidatePhoto(candidate);
            const imagePart = photoUrl
              ? `<img src="${photoUrl}" alt="Foto de ${candidate.name}" loading="lazy">`

...
const waitForSplashTimeout = (promise, timeoutMs = splashGateStepTimeoutMs) =>
  Promise.race([
    ...
    waitForSplashDelay(timeoutMs)
  ]);

const clearSplashFailsafe = () => {
  if (!window.__CATALOGO_SPLASH_FAILSAFE__) {
    return;
  }

  window.clearTimeout(window.__CATALOGO_SPLASH_FAILSAFE__);
  window.__CATALOGO_SPLASH_FAILSAFE__ = 0;
};

const waitForSplashEvent = (eventName, timeoutMs = splashGateStepTimeoutMs) =>
  new Promise((resolve) => {
    let finished = false;
    const finish = () => {
      if (finished) {
...
const waitForSplashDocumentComplete = (timeoutMs = splashGateStepTimeoutMs) => {
  if (document.readyState === "complete") {
    return Promise.resolve();
  }

  return waitForSplashEvent("load", timeoutMs);
};

const waitForSplashFontsReady = (timeoutMs = splashGateStepTimeoutMs) => {
...
const waitForSplashPreloads = (timeoutMs = splashCompactViewportQuery.matches ? 9000 : 11000) => {
  ...
  return waitForSplashTimeout(
    Promise.all(preloadPromises.map((promise) => promise.catch(() => null))),
    timeoutMs
  );
};
...
const waitForSplashDeferredBoot = (timeoutMs = splashCompactViewportQuery.matches ? 8500 : 10000) => {
  ...
  const deferredPromise = window.__CATALOGO_DEFERRED_BOOT_PROMISE__;
  const eventPromise = waitForSplashEvent("catalogo:deferred-assets-ready", timeoutMs);

  if (deferredPromise && typeof deferredPromise.then === "function") {
    return waitForSplashTimeout(Promise.race([deferredPromise, eventPromise]), timeoutMs);
  }
...
const waitForSplashCriticalImages = (timeoutMs = splashGateStepTimeoutMs) => {
  const viewportHeight = window.innerHeight || document.documentElement.clientHeight || 900;
  const seen = new Set();
  const criticalImages = [...document.images]
    .filter((image) => {
      ...
      const imageMode = (image.getAttribute("load" + "ing") || "").toLowerCase();
      if (imageMode === "lazy") {
        return false;
      }
      ...
      if (
        image.closest(
          ".site-header, .hero-newsroom-shell, .hero-insiders-shell, .main-layout, .section-block:first-of-type"
        )
      ) {
        return true;
      }
  ...
  return waitForSplashTimeout(
    Promise.all(criticalImages.map((image) => waitForSplashImage(image))),
    timeoutMs
  );
};

const updateSplashProgress = (progress, status = "") => {
  if (!splashRoot) {
    return;
  }

  const previousProgress = Number(splashRoot.dataset.progress || 0);
  const safeProgress = Math.max(
    previousProgress,
    Math.min(100, Math.max(0, Math.round(progress)))
  );

  splashRoot.dataset.progress = String(safeProgress);
  splashRoot.style.setProperty("--splash-progress", `${safeProgress}%`);

  if (splashProgressBar) {
    splashProgressBar.style.width = `${safeProgress}%`;
  }

  if (splashPercent) {
    splashPercent.textContent = `${safeProgress}%`;
  }
...
const setupSplashExperience = () => {
  ...
  if (!liveSplashMode && splashRoot?.classList.contains("catalogo-cinematic-safe")) {
    ...
    splashRoot.classList.remove("is-logo-primer", "is-broadcast-started", "is-completing", "is-leaving");
  ...
  const handleDeferredBootProgress = (event) => {
...
const getHeroTourismDatasetSignature = () => {
  const items = Array.isArray(window.NEWS_DATA) ? window.NEWS_DATA : [];
  return items
    .slice(0, 36)
    .map((article) =>
      [
        getArticleUsageKey(article),
        getArticleDateKey(article),
        normalizeText(article?.category || article?.categoryKey || "")
      ]
        .filter(Boolean)
        .join(":")
    )
...
const buildDailyOrderedHeroItems = (items = [], seedKey = getHeroTourismDayKey()) => {
  const sourceItems = Array.isArray(items) ? items.filter(Boolean) : [];
  if (sourceItems.length <= 1) {
    return sourceItems;
  }

  const seed = hashHeroTourismSeed(seedKey);
  const buckets = new Map();
  sourceItems.forEach((item) => {
    const areaKey = getHeroAreaKey(item) || normalizeText(item.category || "") || "geral";
    if (!buckets.has(areaKey)) {
      buckets.set(areaKey, []);
    }
    buckets.get(areaKey).push(item);
  });

  const bucketKeys = [...buckets.keys()].sort((left, right) => {
    const leftPriority = heroDailyThemeOrder.indexOf(left);
    const rightPriority = heroDailyThemeOrder.indexOf(right);
    const safeLeftPriority = leftPriority >= 0 ? leftPriority : 99;
    const safeRightPriority = rightPriority >= 0 ? rightPriority : 99;
    if (safeLeftPriority !== safeRightPriority) {
      return safeLeftPriority - safeRightPriority;
    }
    return left.localeCompare(right, "pt-BR");
  });
  const start = bucketKeys.length ? seed % bucketKeys.length : 0;
  const rotatedKeys = [...bucketKeys.slice(start), ...bucketKeys.slice(0, start)];
  const ordered = [];
  let guard = 0;

  while (ordered.length < sourceItems.length && guard < sourceItems.length * 2) {
  ...
  return ordered.length ? ordered : sourceItems;
};
...
const buildHeroTourismDailyPool = () => {
  ...
  const signature = getHeroTourismDatasetSignature();
  if (
    heroTourismDailyPoolState.dayKey === dayKey &&
    heroTourismDailyPoolState.signature === signature &&
    heroTourismDailyPoolState.items.length
  ) {
  ...
  const pushUniquePhoto = (photo = {}) => {
    const sourceKey = getHeroStoryKey(photo) || String(photo.proxyUrl || photo.fallbackUrl || photo.file || photo.title || "").trim();
...
const getBuzzHeatWord = (value = 0) => {
  const percent = clampBuzzPercent(value, 0, 100);
  if (percent >= 72) return "quente";
  if (percent >= 54) return "morno";
  return "frio";
};

const getBuzzHeatFeminineWord = (value = 0) => {
  const word = getBuzzHeatWord(value);
  if (word === "frio") return "fria";
  if (word === "morno") return "morna";
  return word;
};
...
const getDirectSocialTrendStrength = (signalItem = {}) => {
  const title = String(signalItem.title || signalItem.hashtags?.[0] || "").trim();
  const text = normalizeText(
    [
      title,
      signalItem.summary,
      signalItem.category,
      signalItem.importanceDivision,
      signalItem.socialPlatform,
      signalItem.topicGroup
    ].join(" ")
  );
  let score = 34;

  if (/\b(facebook|instagram|tiktok|twitter|x\/twitter)\b/.test(text)) score += 12;
  if (/\b(polem|debate|repercuss|viral|meme|fal[aou]|critica|cobra|inimigo|divid)\b/.test(text)) score += 24;
  if (/\b(acre|jurua|juru[aá]|cruzeiro do sul|czs|rio branco|amazonia|brasil)\b/.test(text)) score += 18;
  if (/\b(politica|política|cultura|futebol|saude|saúde|servico|serviço|evento|show|cidade|bairro)\b/.test(text)) score += 10;

  const engagement = Number(signalItem.facebookEngagement || signalItem.engagement || 0);
  if (engagement > 0) {
    score += Math.min(24, Math.round(Math.log10(engagement + 1) * 10));
  }
...
const buildDirectSocialTrendBuzzArticles = (signalItems = [], limit = 6) => {
  ...
  const fallbackImages = dailyBuzzFallbackImages.length
    ? dailyBuzzFallbackImages
    : ["./assets/home-cache/buzz-cruzeiro-01.jpg"];

  return (Array.isArray(signalItems) ? signalItems : [])
    .map((item) => normalizeRuntimeArticle(item))
    .filter((item) => item.title || item.hashtags?.[0])
    .sort((left, right) => getDirectSocialTrendStrength(right) - getDirectSocialTrendStrength(left))
    .reduce((selected, item, index) => {
      if (selected.length >= limit) {
        return selected;
      }
      ...
      seen.add(key);
      const platform = item.socialPlatform || item.platform || "Rede social";
      const sourceName = item.sourceName || platform;
      const sourceUrl = item.sourceUrl || item.url || item.link || "./arquivo.html";
      const strength = getDirectSocialTrendStrength(item);
      const evidenceKind = /facebook-public|facebook-pages|facebook-posts/.test(normalizeText(item.topicGroup || ""))
        ? "public-post"
        : "trend-signal";
      const category = item.importanceDivision || item.category || "Tendências";
      const hashtags = Array.isArray(item.hashtags) ? item.hashtags.slice(0, 4) : [];
      ...
      selected.push({
        ...
        sourceImageUrl: item.sourceImageUrl || item.imageUrl || fallbackImages[index % fallbackImages.length],
        externalSource: true,
        socialPlatform: platform,
        topicGroup: item.topicGroup || "social-trends",
        socialEvidence: {
          title,
          ...
          evidenceKind,
          engagement: Number(item.facebookEngagement || item.engagement || strength),
          opinionStats: getSocialOpinionStats(item),
          matchScore: strength
        }
      });
      return selected;
...
const buzzOpinionThemeDetails = {
  ...
  default: {
    ...
    route: "dar clima sem fingir certeza: explicar o que importa, para quem muda algo e qual pergunta ainda está em aberto"
  }
};
...
const buildBuzzAgentVoice = (agent = {}, detail = buzzOpinionThemeDetails.default, context = {}) => {
  ...
  const roleLabels = {
    editor: "por que importa",
    sources: "base do caso",
    social: "clima de rede",
    copy: "tom público",
    review: "o que não está claro",
    design: "como aparece",
    sales: "valor para o leitor",
    ceo: "linha principal"
  };
  const label = roleLabels[role] || "leitura de agente";
  const texts = {
    editor: `A conversa importa quando sai do barulho e mostra efeito real: ${detail.context}.`,
    sources: `A base precisa ficar clara: quem publicou, quando publicou e qual fato sustenta a repercussão.`,
    social: hasSocialEvidence
      ? `${networkName} tem conversa viva; apoio, ironia e cobrança precisam aparecer separados.`
      : `Ainda não há rede aberta suficiente para chamar isso de maioria do público.`,
    copy: `A chamada pode ter clima, mas não pode vender conclusão maior que o fato publicado.`,
    review: `O ponto fraco é o que a conversa ainda não respondeu ou ainda não mostrou em público.`,
    design: `O leitor precisa bater o olho e entender se o caso está quente, morno ou frio.`,
    sales: `O valor é deixar claro o que muda na rotina, no consumo, na agenda ou na imagem de quem está no centro.`,
    ceo: `${sourceName} abre a base; a leitura pública só sobe de tom quando a reação aparece com sinal real.`
  };
...
const buildBuzzAudiencePulse = (article = {}, networkContext = {}, index = 0, agentPulse = null) => {
  ...
  const baseSignalStrength = hasSocialEvidence
    ...
    : hasTrendSignal
      ...
      : clampBuzzPercent(Math.max(trustSignal, utilitySignal) - 22, 24, 52);
  ...
  if (evidenceKind !== "public-post") {
    const journalisticVoices = [
      ...
      {
        label: "o que não está claro",
        text: hasTrendSignal
          ? `Ainda não dá para dizer se a reação é apoio, crítica, ironia ou só repetição do tema.`
          : `Ainda falta conversa pública aberta para saber se o público comprou, rejeitou ou apenas observou o caso.`,
        weight: clampBuzzPercent(baseSignalStrength, 24, 78)
      },
      {
        label: "por que importa",
        text: `Importa porque toca ${detail.context}; sem essa leitura, "${subject}" vira só manchete sem termômetro de rua.`,
        weight: clampBuzzPercent(baseSignalStrength - 8, 20, 72)
      }
    ];
    const heatWord = getBuzzHeatWord(baseSignalStrength);
  ...
  if (opinionStats?.sampledComments) {
    ...
    const heatWord = getBuzzHeatWord(Math.max(approvalPercent, rejectionPercent, cautionPercent));
  ...
  return {
    ...
    voices: [
      {
        label: "o que estão falando",
        text: `${socialTitle} puxou interação aberta e ficou ligado a uma notícia real antes de entrar na home.`,
        weight: clampBuzzPercent(baseSignalStrength + 10, 28, 86)
      },
      {
        label: "o que não estão dizendo",
      ...
      {
        label: "por que importa",
        text: `Sem ler os comentários, "${subject}" ainda é calor de repercussão, não sentença sobre a opinião do público.`,
        weight: clampBuzzPercent(baseSignalStrength - 8, 20, 72)
      }
    ]
  };
...
const syncNewsDataset = (runtimeItems = []) => {
  ...
  heroTourismDailyPoolState.dayKey = "";
  heroTourismDailyPoolState.signature = "";
  heroTourismDailyPoolState.items = [];
...
const hydrateDynamicNews = async () => {
  try {
    const preloadedNewsPromise = window.__CATALOGO_NEWS_PRELOAD__;
    window.__CATALOGO_NEWS_PRELOAD__ = null;
    const payload =
      preloadedNewsPromise && typeof preloadedNewsPromise.then === "function"
        ? await preloadedNewsPromise
        : await requestApiJson("/api/news", { method: "GET" });

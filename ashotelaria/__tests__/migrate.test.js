...
test("runMigrations applies SQL and records its name in the same transaction", async () => {
  ...
  const migrationSqlIndexes = queries
    .map(({ text }, index) => (/CREATE TABLE tenants|CREATE TABLE credential_profiles|CREATE TABLE room_photos|CREATE TABLE client_partners|guest_access_code/.test(text) ? index : -1))
    .filter((index) => index >= 0);
  assert.equal(migrationSqlIndexes.length, 5);
  ...
  assert.equal(queries.filter(({ text }) => /pg_advisory_xact_lock/.test(text)).length, 5);
});

...
export function openMediaEditor(root, media, apiClient) {
  ...
  const previewNode = panel.querySelector('.editor-preview img, .editor-preview video');
  const updatePreview = () => applyPreviewTransform(previewNode, buildPresentationPatch(Object.fromEntries(new FormData(form))));
  form.querySelectorAll('select, input').forEach((control) => control.addEventListener('input', updatePreview));
  updatePreview();
  ...
  panel.querySelector('[data-reset]').addEventListener('click', () => { for (const [name, value] of Object.entries({ fitMode: 'contain', focalX: 50, focalY: 50, zoom: 1, rotation: 0, backgroundColor: '#000000' })) form.querySelector(`[name=${name}]`).value = value; updatePreview(); });

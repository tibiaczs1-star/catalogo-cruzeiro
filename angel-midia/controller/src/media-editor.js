...
export function openMediaEditor(root, media, apiClient) {
  ...
  panel.innerHTML = `<form method="dialog"><header><div><span class="media-type">${media.type.startsWith('video/') ? 'VÍDEO' : 'IMAGEM'}</span><h2>${media.name}</h2><p>${media.width || '—'}×${media.height || '—'} · ${media.hasAudio ? 'com áudio' : 'sem áudio'}</p></div><button value="cancel" aria-label="Fechar">×</button></header><div class="editor-layout"><div class="editor-preview"><div><span>Prévia da mídia</span></div></div><div class="editor-controls"><label>Modo de ajuste<select name="fitMode" aria-label="Modo de ajuste"><option value="contain">Conter inteira</option><option value="cover">Preencher e cortar</option><option value="fill">Esticar</option></select></label><label>Centralização horizontal<input name="focalX" aria-label="Centralização horizontal" type="range" min="0" max="100" value="${p.focalX}"></label><label>Centralização vertical<input name="focalY" type="range" min="0" max="100" value="${p.focalY}"></label><label>Zoom<input name="zoom" type="range" min="0.25" max="4" step="0.05" value="${p.zoom}"></label><label>Rotação<select name="rotation"><option>0</option><option>90</option><option>180</option><option>270</option></select></label><label>Cor de fundo<input name="backgroundColor" type="color" value="${p.backgroundColor}"></label><p><strong>Em uso:</strong> ${media.usage.playlists.map((x) => x.name).join(', ') || 'nenhuma playlist'} · ${media.usage.playingNow.length} TVs agora</p></div></div><footer><button value="cancel" class="ghost">Cancelar</button><button type="button" data-reset class="ghost">Restaurar</button><button type="submit" value="save" class="primary">Salvar alterações</button></footer><p role="status"></p></form>`;

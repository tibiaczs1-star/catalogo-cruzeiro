...
export function openMediaEditor(root, media, apiClient, onSaved = () => {}) {
  ...
  panel.querySelector('#media-editor-title').insertAdjacentHTML('afterbegin', `${angelIcon('settings')} `);
  panel.querySelector('[data-align="center"]').innerHTML = angelIcon('center');
  panel.querySelector('[name="zoom"]').closest('label').insertAdjacentHTML('afterbegin', `${angelIcon('zoom')} `);
  panel.querySelector('[name="rotation"]').closest('label').insertAdjacentHTML('afterbegin', `${angelIcon('rotate')} `);
  const simulate = document.createElement('button');
  simulate.type = 'button'; simulate.dataset.simulateTv = ''; simulate.innerHTML = `${angelIcon('tv')} Simular na TV / tela cheia`;

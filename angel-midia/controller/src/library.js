...
export function renderMediaCard(media) {
  ...
  return `<article class="media-card media-card--${sourceType}${count ? ' is-on-air' : ''}" data-media-id="${esc(media.id)}" data-media-source="${sourceType}">
    <div class="media-preview media-preview--${isVideo ? 'video' : 'image'}">${preview}<div class="media-preview-top"><span class="source-badge">${sourceLabel}</span>${count ? `<span class="on-air-badge"><i></i>${count} TV${count === 1 ? '' : 's'}</span>` : ''}</div></div>

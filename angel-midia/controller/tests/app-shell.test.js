...
beforeEach(()=>{localStorage.clear();sessionStorage.clear();delete document.documentElement.dataset.theme;delete document.documentElement.dataset.density;delete document.documentElement.dataset.motion;document.body.innerHTML='<div id="app"></div>'});
...
it('abre a Beta 1.0 com intro Angel e mantém a marca viva na senha',async()=>{const apiClient=vi.fn().mockRejectedValue(new Error('401'));await createApp({root:document.querySelector('#app'),apiClient});const intro=document.querySelector('[data-system-intro]');expect(intro).not.toBeNull();expect(intro.textContent).toContain('BETA 1.0');expect(intro.textContent).toContain('Sua rede vai entrar no ar');expect(intro.querySelector('img').src).toContain('angel-midia-logo.png');expect(intro.querySelectorAll('[data-intro-step]')).toHaveLength(3);expect(document.querySelector('[data-login-logo] img').src).toContain('angel-midia-logo.png');document.querySelector('[data-intro-skip]').click();expect(sessionStorage.getItem('angel-midia:intro:beta-1.0')).toBe('seen');expect(intro.classList.contains('is-leaving')).toBe(true);document.body.innerHTML='<div id="app"></div>';await createApp({root:document.querySelector('#app'),apiClient});expect(document.querySelector('[data-system-intro]')).toBeNull()});
...
it('aplica a identidade azul e branca e mantém manifest instalável',()=>{const manifest=JSON.parse(readFileSync(`${rootDir}/manifest.webmanifest`,'utf8'));const html=readFileSync(`${rootDir}/index.html`,'utf8');expect(manifest.name).toBe('Angel Mídia Play');expect(manifest.theme_color).toBe('#f5f8ff');expect(html).toContain('assets/angel-midia-logo.png');expect(existsSync(`${rootDir}/assets/angel-midia-logo.png`)).toBe(true);expect(html).toContain('data-download="admin"');expect(html).toContain('data-download="tv"');for(const icon of manifest.icons)expect(existsSync(`${rootDir}/${icon.src}`)).toBe(true)});
...
it('usa o sistema visual Angel azul e branco e protege o layout responsivo',()=>{const css=readFileSync(`${rootDir}/src/styles.css`,'utf8')+readFileSync(`${rootDir}/src/responsive.css`,'utf8');expect(css).toContain('--angel-blue:#0f5eea');expect(css).toContain('--angel-surface:#ffffff');expect(css).toContain('min-width:0');expect(css).toContain('overflow-x:clip');expect(css).toContain('@media (prefers-reduced-motion:reduce)');expect(css).toContain('.sidebar{width:');expect(css).toContain('.operations-ribbon');expect(css).toContain('.company-photo-input')});
it('renderiza a central de operações aprovada sem os cards antigos',async()=>{const apiClient=vi.fn(async path=>path==='/auth/me'?{name:'Admin'}:[]);await createApp({root:document.querySelector('#app'),apiClient});expect(document.querySelector('.operations-ribbon')).not.toBeNull();expect(document.querySelector('[data-network-map]')).not.toBeNull();expect(document.querySelector('[data-tv-inventory]')).not.toBeNull();expect(document.querySelector('[data-company-gallery]')).not.toBeNull();expect(document.querySelector('[data-publish-network]')).not.toBeNull();expect(document.querySelectorAll('[data-map-device]')).toHaveLength(3);expect(document.querySelector('.action-card')).toBeNull();expect(document.body.textContent).toContain('Dados de demonstração local')});
it('carrega o retrato NOC da organização e injeta o escopo nos comandos',async()=>{const organizationId='11111111-1111-4111-8111-111111111111';let nocLoads=0;const apiClient=vi.fn(async(path,options)=>{if(path==='/auth/me')return{name:'Admin'};if(path==='/admin/organizations')return{organizations:[{id:organizationId,name:'Matriz Juruá'}]};if(path==='/admin/devices')return{devices:[{id:'tv-real',name:'TV Jornal',company:'Catálogo CZS',latitude:-7.63,longitude:-72.67}]};if(path===`/admin/noc?organizationId=${organizationId}`){nocLoads+=1;return{generatedAt:'2026-08-25T12:00:00.000Z',devices:[{id:'tv-real',health:'unstable',currentMediaName:'Jornal CZS Agora',appVersion:'2.4.0',lastSeenAt:'2026-08-25T11:59:00.000Z'}]}}if(path==='/admin/devices/tv-real/remote-commands')return{status:'queued',...options?.body};return[]});await createApp({root:document.querySelector('#app'),apiClient});expect(apiClient).toHaveBeenCalledWith(`/admin/noc?organizationId=${organizationId}`);expect(document.querySelector('[data-noc-kpi="unstable"] strong').textContent).toBe('1');expect(document.body.textContent).toContain('Jornal CZS Agora');document.querySelector('[data-overview-device="tv-real"]').click();document.querySelector('[data-noc-command="refresh_sync"]').click();await vi.waitFor(()=>expect(apiClient).toHaveBeenCalledWith('/admin/devices/tv-real/remote-commands',{method:'POST',body:{organizationId,commandType:'refresh_sync',idempotencyKey:expect.any(String)}}));document.querySelector('[data-noc-refresh]').click();await vi.waitFor(()=>expect(nocLoads).toBe(2))});

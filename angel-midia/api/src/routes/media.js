    ...
    const { rows } = await app.db.query(
      `with authorized_media as (
         select distinct ma.id,ma.storage_key,ma.content_type,ma.size_bytes
           ...
           join schedules s on s.playlist_id=pi.playlist_id and (s.mode = 'continuous' or (s.starts_at <= now() and s.ends_at > now()))
         ...
         select distinct ma.id, ma.storage_key, ma.content_type, ma.size_bytes
           ...
           join schedules s on s.campaign_id = c.id and (s.mode = 'continuous' or (s.starts_at <= now() and s.ends_at > now()))
           join schedule_targets st on st.schedule_id = s.id
          where ma.id = $1 and (
            st.target_type = 'all'

...
const SOURCE_TYPES = new Set(['owned', 'direct', 'programmatic', 'editorial']);
const CONTENT_KINDS = new Set(['standard', 'advertisement', 'news', 'meme']);

function normalizeContentKind(sourceType = 'owned', contentKind) {
  if (sourceType === 'direct' || sourceType === 'programmatic') return 'advertisement';
  if (sourceType === 'editorial') return contentKind === 'meme' ? 'meme' : 'news';
  return 'standard';
}
...
export function validLibraryMetadata(fields) {
  ...
  if (!SOURCE_TYPES.has(fields.sourceType ?? 'owned')) return false;
  if (fields.contentKind !== undefined && !CONTENT_KINDS.has(fields.contentKind)) return false;
...
function mediaDetail(row) {
  return {
    ...
    sourceType: row.source_type ?? 'owned',
    contentKind: row.content_kind ?? 'standard',
    presentation: {
...
export default async function libraryRoutes(app, { mediaDir, removeMedia = removeStoredMedia } = {}) {
  await app.register(multipart, { limits: { files: 1, fileSize: MAX_MEDIA_BYTES, fields: 4, parts: 5 } });

  app.get('/api/admin/media', { preHandler: requireAdmin }, async () => {
    const { rows } = await app.db.query(`select ma.id,coalesce(ma.display_name,ma.original_name) as name,ma.original_name,ma.content_type,ma.size_bytes,ma.sha256,ma.duration_seconds,ma.processing_status,ma.created_at,ma.width,ma.height,ma.has_audio,ma.source_type,ma.content_kind,
  ...
  app.get('/api/admin/media/:id', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const playingNow = await app.db.query(`select d.id as device_id,d.name as device_name,l.label as location_name from playback_status ps join devices d on d.id=ps.device_id left join locations l on l.id=d.location_id where ps.current_asset_id=$1 and ps.updated_at >= now() - interval '2 minutes' order by d.name`, [request.params.id]);
    return {
  ...
  app.post('/api/admin/media', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    try {
      for await (const part of request.parts()) {
        ...
        } else if (['name', 'durationSeconds', 'sourceType', 'contentKind'].includes(part.fieldname)) fields[part.fieldname] = part.value;
      ...
      const sourceType = fields.sourceType ?? 'owned';
      const contentKind = normalizeContentKind(sourceType, fields.contentKind);
      await app.db.query(`insert into media_assets (id,campaign_id,storage_key,original_name,display_name,content_type,size_bytes,sha256,duration_seconds,source_type,content_kind) values ($1,null,$2,$3,$4,$5,$6,$7,$8,$9,$10)`, [id, stored.storageKey, stored.originalName, fields.name.trim(), stored.contentType, stored.sizeBytes, stored.sha256, duration, sourceType, contentKind]);
      return reply.code(201).send({ id, name: fields.name.trim(), type: stored.contentType, sizeBytes: stored.sizeBytes, sha256: stored.sha256, durationSeconds: duration, sourceType, contentKind, status: 'ready' });
    } catch (error) {

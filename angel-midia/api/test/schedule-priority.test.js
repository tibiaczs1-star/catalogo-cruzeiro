...
test('manifest preserves the winning playlist order and playback metadata', async () => {
  let sql = ''; const db = { query: async (query) => { sql = query; return { rows: [
  ...
  assert.match(sql, /coalesce\(pi\.image_duration_seconds,\s*asset\.duration_seconds,\s*legacy\.duration_seconds\)/i);
});

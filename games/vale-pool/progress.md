...
- Rodada poolturn1: correção de execução do Par/Impar. Falta de bola branca passa a vez para IA/rival em Demo e PvP; cartões/HUD deixam claro `VOCE/RIVAL/IA: PAR/IMPAR`; apos a escolha da moeda aparece animacao `MODO ESCOLHIDO` antes do tutorial. Evidencias: `.codex-temp/vale-pool-poolturn1/mode-reveal.png` e `.codex-temp/vale-pool-poolturn1/tutorial.png`.
- Rodada poolmobileintro1: modo Livre encerra quando nao restam bolas de ataque, falta da branca virou `bola na mão` com posicionamento antes da tacada, PvP envia `cueX/cueY`, mobile/touch passou para fluxo mira -> abre força -> taca, e a entrada do jogo ganhou animacao de taco batendo na bola e abrindo a mesa. Evidencias: `.codex-temp/vale-pool-poolmobileintro1/intro.png` e `.codex-temp/vale-pool-poolmobileintro1/mobile-power-stage.png`.
- Rodada pooltouchaim1: controle mobile da Sinuca separou arrasto de toque. Arrastar/segurar no canvas agora apenas ajusta a mira e suprime o click sintetico que iniciava a força sem querer; toque curto parado continua avançando para força/tacada. O canvas recebeu `touch-action: none` para o navegador nao roubar o gesto. Validacao: `node --check`, `npm run guard:pubpaid`, web_game_playwright_client e Playwright touch controlado confirmaram arrasto ficando em `mira` e toque curto indo para `forca`. Evidencia: `.codex-temp/vale-pool-pooltouchaim1/demo-touch-controlled.png`.

...
(function () {
  ...
  function createModulesRow() {
    ...
    wrap.innerHTML = MODULES.map(
      (module) =>
        `<a class="servicos-modulo-link" href="catalogo-servicos.html?modulo=${encodeURIComponent(
          module.id
        )}">${module.label}</a>`
    ).join("");
    return wrap;
  ...
  function initPhonebookRedirects() {
    ...
    const clickable = Array.from(
      document.querySelectorAll("button, a, .chip, .tag, .filter, .pill, span")
    );
